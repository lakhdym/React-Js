...
const Home = {
	...
	computed: {
		filterList() {
			return this.products.filter(product => {
				// filter avec description  et prix
				// return true or false
				this.favoris = 15;
				if (product.price.includes(this.searchprice) &&  product.description.toLowerCase().includes(this.searchkey.toLowerCase())) {
		...
		/*addtcurrency(){
			return this.searchprice = this.searchprice + '$';
		}*/
	...
	methods: {
		getData: function () {
			//this.products = [];
			axios.post('/vue-project-starting_file/php/action.php', {
				action: 'fetchall'
			}).then(response => {
